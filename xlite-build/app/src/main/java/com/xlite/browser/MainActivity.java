...
public class MainActivity extends Activity {
    ...
    private void createSession() {
        ...
        session.setPromptDelegate(new GeckoSession.PromptDelegate() {
            ...
            public GeckoResult<PromptResponse> onFilePrompt(
                ...
                pick.setType(
                        prompt.mimeTypes != null && prompt.mimeTypes.length == 1
                                ? prompt.mimeTypes[0]
                                : "*/*"
                );
                ...
                new WebExtension.MessageDelegate() {
                    ...
                    public GeckoResult<Object> onMessage(
                        ...
                        if (message instanceof JSONObject) {
                            ...
                            if ("openMenu".equals(type)) {
                                runOnUiThread(MainActivity.this::showMenu);
                                return GeckoResult.fromValue(Boolean.TRUE);
                            }

                            if ("openControlPanel".equals(type)) {
                                runOnUiThread(MainActivity.this::openControlPanelOptions);
                                return GeckoResult.fromValue(Boolean.TRUE);
                            }
                        }
                        ...
                        try {
                            ...
                            reply.put("performanceMode", prefs.getBoolean("performance_mode", true));
                        } catch (Exception ignored) {
                        }
    ...
    private void showMenu() {
        ...
        boolean performance = prefs.getBoolean("performance_mode", true);
                ...
                .setItems(items, (d, which) -> {
                    switch (which) {
                        ...
                        case 6:
                            ...
                            break;
                        case 7:
                            prefs.edit().putBoolean("performance_mode", !performance).apply();
                            session.reload();
                            break;
                        case 8:
    ...
    private void openControlPanelOptions() {
        if (controlPanel != null
            ...
            Toast.makeText(
                    this,
                    "Control Panel ainda está inicializando.",
                    Toast.LENGTH_SHORT
            ).show();
        }
    }
    ...
    private void showVideoQualityDialog() {
        ...
        new AlertDialog.Builder(this)
                ...
                .setSingleChoiceItems(
                        ...
                        (d, which) -> {
                            prefs.edit()
                                    .putInt("video_max_height", which == 1 ? 1080 : 720)
                                    .apply();
                            d.dismiss();
                            session.reload();
                        }
    ...
    private void openExternalCurrentPage() {
        try {
            startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(currentUri)));
        } catch (Exception ignored) {
        }
    }
