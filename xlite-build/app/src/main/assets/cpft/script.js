...
if ($settings) {
  try {
    Object.assign(config, JSON.parse($settings.innerText)); config.bypassAgeVerification = false
  } catch(e) {
  ...
  let settingsChangeObserver = new MutationObserver(() => {
    ...
    Object.assign(config, configChanges); config.bypassAgeVerification = false
