...
class VideoOptionsBottomSheet : BaseBottomSheet() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val isInWatchLater = runBlocking(Dispatchers.IO) {
            WatchLaterHelper.contains(videoId)
        }
        optionsList += R.string.addToPlaylist
        optionsList += if (isInWatchLater) R.string.remove_watch_later else R.string.add_watch_later
        optionsList += listOf(R.string.download, R.string.share)
        ...
        setSimpleItems(optionsList.map { getString(it) }) { which ->
            when (optionsList[which]) {
                ...
                R.string.add_watch_later -> {
                    withContext(Dispatchers.IO) {
                        WatchLaterHelper.add(streamItem)
                    }
                    setFragmentResult(VIDEO_OPTIONS_SHEET_REQUEST_KEY, bundleOf())
                }

                R.string.remove_watch_later -> {
                    withContext(Dispatchers.IO) {
                        WatchLaterHelper.remove(videoId)
                    }
                    setFragmentResult(VIDEO_OPTIONS_SHEET_REQUEST_KEY, bundleOf())
                }
