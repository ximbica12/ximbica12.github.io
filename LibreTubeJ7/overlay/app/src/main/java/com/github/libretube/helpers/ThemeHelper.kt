...
object ThemeHelper {
    ...
    fun updateTheme(activity: AppCompatActivity) {
        ...
        val migratedAccent = PreferenceHelper.getBoolean("nexotube_accent_migrated_v1", false)
        if (!migratedAccent) {
            if (accentColor.isEmpty() || accentColor == "blue") {
                accentColor = if (DynamicColors.isDynamicColorAvailable()) "my" else "red"
                PreferenceHelper.putString(PreferenceKeys.ACCENT_COLOR, accentColor)
            }
            PreferenceHelper.putBoolean("nexotube_accent_migrated_v1", true)
    ...
    fun getStyledAppName(context: Context): Spanned {
        ...
        return "Nexo<span  style='color:$hexColor';>Tube</span>"
